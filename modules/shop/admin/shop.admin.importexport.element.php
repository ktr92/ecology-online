...
class Shop_admin_importexport_element extends Frame_admin
{
		...
		'main' => array (
			...
			'type' => array(
				...
				'select' => array(
					...
					'view_element' => 'Шаблон модуля для вложенных товаров (modules/shop/views/shop.view.шаблон.php)', // (modules/shop/views/shop.view.шаблон.php)
	...
	public function show()
	{
		...
		if ($this->diafan->count)
		{
			...
			<div class="box box_half box_height box_right">
				...
				<a href="'.str_replace('/'.ADMIN_FOLDER, '', BASE_PATH_HREF).'shop/export/'.$this->diafan->_route->cat.'/?'.rand(0, 999999).'" class="file-load">
	...
	public function edit_variable_paramhelp()
	{
		...
		↑ <a href="http://www.diafan.ru/dokument/full-manual/modules/shop/#Import/eksport-YA.Market" target="_blank">'.$this->diafan->_('О типах полей для импорта').'</a>
		</div>';
	}
