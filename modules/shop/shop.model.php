...
class Shop_model extends Model
{
	...
	private function list_search_query($where_param, $where, $group, $values)
	{
		...
		."s.hit, s.new, s.action, s.is_file, s.[measure_unit] FROM {shop} AS s"
		.($this->diafan->configmodules('where_access_element') ? " LEFT JOIN {access} AS a ON a.element_id=s.id AND a.module_name='shop' AND a.element_type='element'" : "")
		.($this->diafan->configmodules('hide_missing_goods') && $this->diafan->configmodules('use_count_goods') ? " INNER JOIN {shop_price} AS prh ON prh.good_id=s.id AND prh.count_goods>0" : "")
		.$where_param
		.($this->sort_config['use_params_for_sort'] ? " LEFT JOIN {shop_param_element} AS sp  ON sp.element_id=s.id AND sp.trash='0' AND sp.param_id=".$this->sort_config['param_ids'][$this->diafan->_route->sort] : '')
	...
	public function list_param()
	{
		...
		{
			if ( ! $param_value = DB::query_fetch_array("SELECT param_id, [name] FROM {shop_param_select} WHERE id=%d LIMIT 1", $this->diafan->_route->param))
			{
				Custom::inc('includes/404.php');
			}
			if ( ! $param = DB::query_fetch_array("SELECT [name] FROM {shop_param} WHERE id=%d LIMIT 1", $param_value["param_id"]))
			{
				Custom::inc('includes/404.php');
			}
			////navigation//
	...
	private function list_param_query($time, $param_id)
	{
		...
		$rows = DB::query_range_fetch_all(
			"SELECT s.id, s.[name], s.timeedit, s.[anons], s.site_id, s.brand_id, s.no_buy,  s.article, s.[measure_unit], s.is_file FROM {shop} AS s"
	...
	private function first_page_elements_query($time, $cat_ids)
	{
		...
		." e.[measure_unit], e.new, e.action, e.is_file FROM {shop} AS e"
	...
	private function list_category_query()
	{
		...
		$row = DB::query_fetch_array("SELECT id, [name], [anons], [anons_plus] ".$fields.", timeedit, [descr], [keywords], [canonical], sort, parent_id, [title_meta], access, theme, view, [act], noindex FROM {shop_category}"
	 ...
	 * @return array
	 */
	public function show_block($count, $site_ids, $cat_ids, $brand_ids, $sort, $images, $images_variation, $param, $hits_only, $action_only, $new_only, $discount_only, $tag)
	{
		$time = mktime(23, 59, 0, date("m"), date("d"), date("Y"));
	...
	public function show_category($site_ids, $images, $images_variation, $count_level, $number_elements)
	{
		...
		if (! $this->result = $this->diafan->_cache->get($cache_meta, "shop"))
		{
			...
			{
				$rng = DB::query_fetch_key_value("SELECT cat_id, COUNT(*) AS count FROM {shop_category_rel} GROUP BY cat_id", "cat_id", "count");
